...
def test_login_happy_path(login_routes):
    with CronometerClient() as client:
        session = client.login("me@example.com", "s3cret")
...
def test_login_missing_csrf_cookie(login_routes):
    ...
    with CronometerClient() as client, pytest.raises(LoginError, match="anti-CSRF"):
        client.login("me@example.com", "pw")
...
def test_login_bad_credentials_no_sesnonce(login_routes):
    ...
    with CronometerClient() as client, pytest.raises(LoginError, match="bad credentials"):
...
def test_login_http_error(login_routes):
    ...
    with CronometerClient() as client, pytest.raises(LoginError, match="HTTP 500"):
        client.login("me@example.com", "pw")
...
def test_login_gwt_authenticate_failure(login_routes):
    ...
    with CronometerClient() as client, pytest.raises(LoginError, match="GWT authenticate failed"):
        client.login("me@example.com", "pw")
...
def test_login_gwt_hash_not_found(login_routes):
    ...
    with CronometerClient() as client, pytest.raises(CronometerError, match="permutation"):
        client.login("me@example.com", "pw")
...
def test_search_params_and_result(api, session):
    ...
    with CronometerClient(session) as client:
        result = client.search("chili", max_results=7, sources="Custom")

    assert result == payload
    params = route.calls.last.request.url.params
    assert params["query"] == "chili"
    assert params["maxResults"] == "7"
    assert params["sources"] == "Custom"
